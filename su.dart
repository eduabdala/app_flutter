...
class _SerialChartAppState extends State<SuChartApp> {
  ...
  void _initializeSerialPort() {
    ...
    if (availablePorts.isNotEmpty) {
      ...
      _logMessage('No available serial ports.');
    }
  }
...
Future<void> getFirmwareVersion() async {
  ...
  try {
    _sendCommandWithText("V");
    if (firmwareVersion == "Unknown") {
      _logMessage('Firmware version could not be determined.');
    } else {
      _logMessage('Firmware version: $firmwareVersion');
      setState(() {
        firmwareVersion = firmwareVersion;
      });
    }
  } catch (e) {
    _logMessage('Error obtaining firmware version: $e');
  }
}
    ...
    if (port != null && isPortOpen) {
      ...
      if (isPdMode && _isSending) {
        ...
        if (commandMode == CommandMode.Hexadecimal) {
          ...
          port!.write(Uint8List.fromList(utf8.encode(fullCommandWithBCC + '\n')));
        ...
        if (commandMode == CommandMode.Hexadecimal) {
          ...
          _logMessage('Controler: ' + commandController.text);
...
Future<void> _onDataReceived(String data) async {
  ...
  if (isPortOpen && _isSending && isPlaying == false) {    
    _logMessage("debug: $data");
  }

  if (isLogging) {
    final downloadsDirectory = await getDownloadsDirectory();
    if (downloadsDirectory == null) {
      _logMessage('Error obtaining Downloads directory.');
      return;
    }
    ...
    try {
      ...
      _logMessage('Error saving data to file: $e');
    }
  }
}
  ...
  void _startSendingStatusCommand() {
    if (port != null && isPortOpen) {
      sendTimer = Timer.periodic(Duration(milliseconds: 500), (timer) {
        if (isPdMode && isPlaying) {
          _sendCommandWithText('T>');
        } else {
  ...
  void _updateChartData(String data) {
    ...
    if (parts.length >= 10 && isPdMode == false && isPlaying == false) {
      List<String> parts = data.split(';');
      setState(() {
      ...
      String cleanedData = data.replaceAll(RegExp(r'^T>#|#$'), '');
      int inicial = 4;
      setState(() {
        // Suponha que você queira adicionar dados a outras listas quando isPdMode é true
        minData.add(ChartData(double.parse(parts[6])));
        midData.add(ChartData(double.parse(parts[7])));
        maxData.add(ChartData(double.parse(parts[8])));
        inMinData.add(ChartData(double.parse(parts[12])));
